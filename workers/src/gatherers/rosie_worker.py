    ...
    def _download_and_parse_full(self, year: int, target_ids: Optional[set] = None) -> List[Dict]:
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                import hashlib
                fornecedor = row_lower.get('txtfornecedor', '').strip()
                hash_fornecedor = hashlib.md5(fornecedor.encode("utf-8")).hexdigest()[:15]
                num_doc = row_lower.get('numdocumento', 'unknown')
                
                # Match the exact ID format generated by expenses_worker.py
                # Which is: f"despesa_{num_doc}_{hash_fornecedor}"
                receipt_id = f"despesa_{num_doc}_{hash_fornecedor}"
    ...
    def _save_anomalies_csv(self, report: Dict, filename: str = "rosie_anomalies.csv"):
        try:
            ...
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                ...
                writer.writeheader()
                for a in anomalies:
                    # Assuming 'is_suspicious' is a key in the anomaly dictionary 'a'
                    # and 'receipt' object is not directly available here,
                    # but receipt_id and deputy_id are part of the anomaly 'a'.
                    # The instruction refers to `_run_analysis` which is not in this file,
                    # so I'm interpreting the intent to push anomalies to backend
                    # from where they are processed in this worker.
                    if a.get("is_suspicious", False): # Check if the anomaly is suspicious
                        # POST anomaly directly to the backend API so it embeds into Neo4j
                        try:
                            # Use self.backend_url for consistency
                            base_url = f"{self.backend_url}/api/internal/workers/ingest/rosie-anomaly"
                            payload = {
                                "receiptId": a.get("receipt_id"),
                                "anomaly": a # Send the full anomaly object
                            }
                            requests.post(base_url, json=payload, timeout=5)
                        except Exception as e:
                            logger.warning(f"Failed to push anomaly to backend for receipt {a.get('receipt_id')}: {e}")
